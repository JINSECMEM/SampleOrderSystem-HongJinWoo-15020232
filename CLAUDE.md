...
- Mock 대상: Database, Service 계층의 인터페이스

**Test Script 작성 기준**
- 주문 상태 전환 시나리오별 테스트 (RESERVED→CONFIRMED, RESERVED→PRODUCING 등)
- 재고 충분/부족 분기 검증
- 생산량 공식(`ceil(부족분 / (수율 × 0.9))`) 경계값 테스트
- 생산 큐 FIFO 순서 보장 검증
- 각 Service 메서드 단위로 테스트 작성 (Database는 Mock으로 대체)

## Clean Code 기준

- 함수는 하나의 책임만 가진다 (Single Responsibility)
- 함수 길이는 40줄 이내를 목표로 한다
- 매직 넘버 사용 금지 — 상수로 명명하여 사용
- 계층 간 직접 접근 금지 (View → Service → Database 순서 준수)
- 구현 후 `/simplify` 스킬로 품질 검토

## 개발 워크플로우

구현 정확도를 높이기 위해 다음 순서를 따른다:

1. **구현 전** — `Plan` 서브에이전트로 파일/클래스 구조 확정 후 진입
   - 여러 계층이 얽히는 기능(생산 큐, 재고-주문 연동 등)에서 특히 적용
2. **구현 후** — `/simplify` 스킬로 코드 품질 검토 (Clean Code 주안점 대응)
