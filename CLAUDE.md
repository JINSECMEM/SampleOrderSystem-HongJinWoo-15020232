...
├── Services/        # 비즈니스 로직
├── Repositories/    # 데이터 접근 계층 (IRepository<T, Key> 기반)
└── Utils/           # 공용 유틸리티 (ConsoleHelper 등)
```

## 코딩 컨벤션

POC 프로젝트들에서 공통으로 확인된 스타일:

| 항목 | 규칙 | 예시 |
|------|------|------|
| 클래스·구조체 | PascalCase | `OrderService`, `ProductionJob` |
| 메서드 | PascalCase | `RegisterOrder()`, `GetAllOrders()` |
| 멤버 변수 | trailing underscore | `orderRepo_`, `running_` |
| 지역 변수 | camelCase | `totalCount`, `newOrder` |
| Enum 값 | UPPER_CASE | `OrderStatus::RESERVED` |
| 헤더 가드 | `#pragma once` | |
| 언어 | C++17 | `std::optional`, `std::filesystem` |
...
별도 테스트 프레임워크 없이 **수동 시나리오 테스트**를 기본으로 한다.
`Tests/` 디렉터리에 시나리오별 입력·기대 출력을 텍스트 파일로 관리한다.

## 참고 POC 경로

| POC | 경로 | 참고 내용 |
|-----|------|-----------|
| MVC 스켈레톤 | `C:\reviewer\ConsoleMVC_gitlocal` | 전체 구조, DI, Repository 패턴 |
